...
const intentsItems = ["clothing","electronic","jewelry","accessory"];
var options = {
    qs : null
};
var queryString ={
...
const setEntityValues =(context,reset) =>{
   ...
   context.context = {};
   //DEtect intent value
    intentsItems.forEach(function(v){
        delete context.context[intentsItems[v]];
    });
   if(context.entities){
    Object.keys(context.entities).forEach(function(key){
            ...
            if(key=='intent'){
                var tempValue = context.entities[key][0].value;
                tempValue = tempValue.toLowerCase();
                context.context[tempValue] = true;     
            }
    ...
    return new Promise(function(resolve,reject){
        request(optionsParam,function(error,response,body){
            if(!error  && response.statusCode ==200){
                ...
                console.log('Error adding an item to basket',body);
                reject(body);
            }
        });
    });
...
const sessionBridge =(cb)=>{
    var optionsParam = {
        uri : entireURL + '/sessions',
        headers : {
            'authorization' : process.env.JWT_TOKEN,
            'origin' : SITE_HOST,
...
module.exports = { searchProductOnDemandWare,retrieveCustomObjectValue,setEntityValues,getJWTToken,createBasket,addToBasket,sessionBridge  };